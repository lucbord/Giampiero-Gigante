...
h=a.getNext(b),j,i,r;f.hide(g.elems.info);try{for(r=a._options.preload;r>0;r--)i=new Galleria.Picture,j=a.getData(h),i.preload("big"in j?j.big:j.image),h=a.getNext(h)}catch(k){}g.image.load(c.big||c.image,function(a){g.width=a.original.width;g.height=a.original.height;e(a.image).css({width:"100.5_25",height:"100.5_25",top:0,zIndex:99998});f.hide(a.image);g.elems.title.innerHTML=c.title||"";g.elems.counter.innerHTML=b+1+" / "+d;u.resize(g.rescale);g.rescale()});e(g.elems.overlay).show();e(g.elems.box).show()}};
